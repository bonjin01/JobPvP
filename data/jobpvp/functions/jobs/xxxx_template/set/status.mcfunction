#> jobpvp:jobs/xxxx_template/set/status
#
# 職業の作成
# ここでステータスを設定する
#
# @within function jobpvp:jobs/xxxx_template/set/

# 職業のIDを入れる(フォルダ名のxxxxの部分)
    #data modify storage jobpvp: job_ID set value 
# 職業の名前
    data modify storage jobpvp: job_name set value '{"text":"職業の名前"}'
# 職業のフレーバーテキスト
