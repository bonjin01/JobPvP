...
execute if data storage jobpvp: lib_point.Inventory[0].tag.CountPerCost store result score $temp.cpc Temporary run data get storage jobpvp: lib_point.Inventory[0].tag.CountPerCost
execute unless data storage jobpvp: lib_point.Inventory[0].tag.CountPerCost run scoreboard players set $temp.cpc Temporary 1
execute if score $temp.cpc Temporary matches ..0 run scoreboard players set $temp.cpc Temporary 1

# 計算し適応
execute store result score $temp.item_costcalc Temporary run scoreboard players operation $temp.count Temporary /= $temp.cpc Temporary
...
scoreboard players operation $temp.item_costcalc_ Temporary *= $temp.cpc Temporary
scoreboard players operation $temp.skill_costcalc_ Temporary = $temp.skill_costcalc Temporary
scoreboard players operation $temp.skill_costcalc_ Temporary *= $temp.cpc Temporary
