...
# Qキーでリロード（仮）
execute as @s[tag=tmw_drop_n] if score $cooltime ui_temp matches 0 run function jobpvp:guns/player/crossbow/reload/top
...
# 射撃管制
execute unless score $firetime ui_temp matches 0 run function jobpvp:guns/player/crossbow/fire/time

# 右クリックでリロード
execute as @s[tag=tmw_active_temp] if score $cooltime ui_temp matches 0 if score $reloadtime ui_temp matches 0 run function jobpvp:guns/player/crossbow/reload/top
