...
# 弾丸
    execute as @e[tag=tmw_255] at @s unless score $bullettime ui_world matches 1.. run function jobpvp:guns/projectile/tick
    execute at @e[tag=tmw_255] if score $bullettime ui_world matches 1.. run particle dust 0 0 0 0.5 ~ ~ ~ 0 0 0 0 1 force

# SSロジック維持
    execute if entity @e[tag=tmw_255] run schedule function jobpvp:guns/player/crossbow/ss/1 1t append