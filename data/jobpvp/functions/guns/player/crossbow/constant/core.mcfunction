# スペ以外の時に実行される常駐効果の管理
    ...
    execute at @s[scores={ui_st=0}] run function jobpvp:guns/player/crossbow/constant/shoot
    execute as @s[scores={ui_st=1..}] at @s run function jobpvp:guns/player/crossbow/constant/sneak