...
# 条件に当てはまる弾を線形探索で探し、スロットを特定する
    ...
    execute if score $max_size ui_temp matches 1.. run function jobpvp:guns/player/crossbow/reload/ui_normal/linear_search
    execute if score $max_size ui_temp matches 0 run function jobpvp:guns/player/crossbow/reload/vanilla/linear_search
    execute if score $max_size ui_temp matches ..-1 run function jobpvp:guns/player/crossbow/reload/independant/linear_search
    #tellraw @a {"score":{"name":"$slot","objective":"ui_temp"},"color":"gold"}

# 弾を消費
    execute unless score $slot ui_temp matches -1 run function jobpvp:guns/player/crossbow/reload/found