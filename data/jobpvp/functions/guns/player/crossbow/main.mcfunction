...
execute if data storage ui:gun temp.ReloadMP run scoreboard players set $reloaditem ui_temp 1

# 初期設定
execute unless data storage ui:gun temp.now.First run function jobpvp:guns/player/crossbow/changed/first
...
# 常駐効果
execute if entity @s[gamemode=!spectator] run function jobpvp:guns/player/crossbow/constant/core

# Qキーでリロード（仮）
execute as @s[tag=tmw_drop_n] if score $cooltime ui_temp matches 0 if score $firetime ui_temp matches 0 run function jobpvp:guns/player/crossbow/reload/top

# Fでスコープ
execute as @s[tag=tmw_drop_s] if score $cooltime ui_temp matches 0 if score $firetime ui_temp matches 0 at @s run function jobpvp:guns/player/crossbow/scope/manager

# クールタイム解除
execute unless score $cooltime ui_temp matches 0 run function jobpvp:guns/player/crossbow/ct

# リロード中
execute unless score $reloadtime ui_temp matches 0 run function jobpvp:guns/player/crossbow/reload/time
...
execute if score $fire ui_temp matches 1 run function jobpvp:guns/player/crossbow/fire/time

# 弾丸の射出
execute if score $fire ui_temp matches 1.. run function jobpvp:guns/player/crossbow/fire/attack

# 逆変換
execute if score $changed ui_temp matches 1 run function jobpvp:guns/player/crossbow/changed/core
...
# 最後に
schedule function jobpvp:guns/player/crossbow/last 1t append